...
scaled_polygon3 = polygons.fdV2_5_polygons.normalized_polygon3 * np.array([image_width, image_height])
scaled_polygon4 = polygons.fdV2_5_polygons.normalized_polygon4 * np.array([image_width, image_height])
...
# Conditional filtering based on the model
if args.model == "1_class":
...
for zone, zone_annotator, box_annotator in zip(zones, zone_annotators, box_annotators):
    ...
    for _, confidence, class_id, tracker_id in detections_filtered:
        if zone.polygon.shape == entrance_polygon.shape:
            zone_name = "Entrance"
        elif zone.polygon.shape == inside_polygon.shape:
            zone_name = "Inside"
        elif zone.polygon.shape == exit_polygon.shape:
            zone_name = "Exit"
        else:
            zone_name = "Unknown"

        labels.append(f"#{tracker_id} {CLASS_NAMES_DICT[class_id]} {confidence:.2f}")  # Add labels
        csv_data.append([tracker_id, CLASS_NAMES_DICT[class_id], zone_name, confidence, xyxy])
